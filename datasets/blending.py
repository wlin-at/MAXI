...
class BaseMiniBatchBlending(metaclass=ABCMeta):
    ...
    def __init__(self, num_classes, smoothing=0.):
        ...
        self.off_value = smoothing / self.num_classes #  smoothing is config.AUG.LABEL_SMOOTH  0.1 ,  off_value is  smoothing/n_class = 0.00025
        self.on_value = 1. - smoothing + self.off_value   #  on_value = 1 - smoothing + off_value  =  1- 0.1 + 0.00025 = 0.90025

    @abstractmethod
    def do_blending(self, imgs, label, **kwargs):
        pass

    def __call__(self, imgs, label, if_soft_labels = False,  **kwargs):
